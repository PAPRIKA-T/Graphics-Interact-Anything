...
struct SamModel {
  ...
  bool loadImage(const cv::Mat& org_image) {
    std::vector<uint8_t> inputTensorValues(inputShapePre[0] * inputShapePre[1] * inputShapePre[2] *
                                           inputShapePre[3]);

    if (org_image.size() != cv::Size(inputShapePre[3], inputShapePre[2])) {
...
bool Sam::loadImage(const cv::Mat& org_image) { return m_model->loadImage(org_image); }
